...
//NRF24L01 驱动函数 
u8 frequency = 0x18;	//24L01频率初始化为24
u8 bandwidth = 0x26;  //带宽初始化为0.25Mbps

unsigned char idel_mode_flag = 0;
unsigned char mode_time_counter = 0;	 
const u8 TX_ADDRESS[TX_ADR_WIDTH]={0x00,0x98,0x45,0x71,0x10}; //发送地址
const u8 RX_ADDRESS[RX_ADR_WIDTH]={0x00+0x11,0x98+0x11,0x45+0x11,0x71+0x11,0x10+0x11}; //Car Receiver Address 地址
//const u8 RX_ADDRESS[RX_ADR_WIDTH]={0x00,0x98,0x45,0x71,0x10}; // Transmitter Receiver Address
//初始化24L01的IO口
...
{
	Clr_NRF24L01_CE;	  
		NRF24L01_Write_Reg( SETUP_AW, 0x3 );
    ...
    //设置TX发射参数,20db增益,0.25Mbps,低噪声增益开启   
  	NRF24L01_Write_Reg(SPI_WRITE_REG+RF_SETUP,bandwidth);
...
{														 
    ...
    //设置TX节点地址,主要为了使能ACK	  
  	NRF24L01_Write_Buf(SPI_WRITE_REG+RX_ADDR_P0,(u8*)RX_ADDRESS,RX_ADR_WIDTH); 
		NRF24L01_Write_Reg( SPI_WRITE_REG+RX_PW_P0, TX_PLOAD_WIDTH );
    //Disable 通道0的自动应答
  	NRF24L01_Write_Reg(SPI_WRITE_REG+EN_AA,0x00);     
    //Disable All 通道的接收地址
  	NRF24L01_Write_Reg(SPI_WRITE_REG+EN_RXADDR,0x00); 
    //设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次
  	NRF24L01_Write_Reg(SPI_WRITE_REG+SETUP_RETR,0);
		//设置RF通道为24
  	NRF24L01_Write_Reg(SPI_WRITE_REG+RF_CH,frequency);
    //设置TX发射参数,20db增益,0.25Mbps,低噪声增益开启   
  	NRF24L01_Write_Reg(SPI_WRITE_REG+RF_SETUP,bandwidth);
    //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,PRIM_RX发送模式,开启所有中断
  	NRF24L01_Write_Reg(SPI_WRITE_REG+CONFIG, ( 1 << 3 ) | //Enable CRC
                                      ( 1 << 1 )| // PWR_UP
																			( 1 << 2)); // 16bit CRC
    // CE为高,10us后启动发送
	Set_NRF24L01_CE;                                  
